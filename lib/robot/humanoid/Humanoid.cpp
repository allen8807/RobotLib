...
////////////////////////////////////////////////
 ...
 //////////////////////////////////////////////
		...
		Vector3f leftFootGlobalAxis,rightFootGlobalAxis;
            list<unsigned int> dofid;
            FOR_EACH( iter, idx ){
                dofid.push_back( (*iter)->joint()->DOF(0).id ); // only consider Hinge Joint here!!
            }
                //unsigned int llj2=getJointSensorId("llj2");
		//unsigned int rlj2=getJointSensorId("rlj2");
            //unsigned int lhip1Id=getBoneId(L_HIP);
            for( int i=0; i<100; i++){
		    ...
		    //added by dpf to let the feet be paralle to the ground!
		    leftFootGlobalAxis=(getBone(L_FOOT)->calcJointGlobalAxisFromBody(getBone(L_FOOT)->getJointGlobalMat()));
		    rightFootGlobalAxis=(getBone(R_FOOT)->calcJointGlobalAxisFromBody(getBone(R_FOOT)->getJointGlobalMat()));
                if ( errlen < 1.0e-10
		  &&abs(asin(leftFootGlobalAxis.z()/leftFootGlobalAxis.length()))<5*3.14/180
		  &&abs(asin(rightFootGlobalAxis.z()/rightFootGlobalAxis.length()))<5*3.14/180
		){
                    return true;
                }
                ...
                //patch by bestdpf
                //unsigned int lhip1Id=getBone(getBoneId(L_HIP))->joint()->DOF(0).axis;
		
		//unsigned int rhip1Id=getBoneId(R_HIP);
            }
            return false;
        }
